...
void usage(const char *pname) {
	printf(	"Usage: %s [OPTION] <deletions.haplotypes> <snps.haplotypes>\n"
		"\n"
		" -m\tNUM\tNumber of SNPs used for imputating the deletion.\n"
		" -s\tNUM\tRange in which SNPs must lie in order to be used for imputation. \n"
		" -v\t\tVerbose.\n"
		" -h\t\tPrint this help.\n\n", 
		pname) ; 
	exit(0) ; 
...
void parse_arguments(parameters* p, char* input_filename1, char* input_filename2, int argc, char **argv) {
	...
	/* set defaults */
	p->number_of_snps = 0 ; // signifies this option is not used
	p->search_range = 1000000 ; 
	p->verbose	= 0 ; 
	
	size_t ch ; 
	while ((ch = getopt(argc, argv, "m:s:vh")) != -1)
    	{
       		switch(ch) {
			case 'm': p->number_of_snps = strtol(optarg, 0, 10); break ; 
	...
	if (p->number_of_snps < 0) {
		printf("ERROR: Invalid argument. number_of_snps (-m) must be nonnegative.\n") ;
		error_occurred = 1 ;  
	}
